...
//    private final ReflectedTypeRef<ReflectedValueClass<R>> ref;
    ...
    public static <T> ReflectedAccessorMethod<T, ?> forMethod(
        ...
        boolean isCollection = ReflectUtil.isCollection(returnType);
        Class<?> returnClass= ReflectUtil.getCollectionClass(returnType);
        //TODO support array and map here too
     

        try {
            ReflectedTypeClass<?> returnModel = DomainClassReflector.model(returnClass);
